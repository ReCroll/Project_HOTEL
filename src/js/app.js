...
//=======================================================================
// //=========================== POSITION PAGINATION ============================
function positionPag(width) {
  let pagination = document.querySelector("._pagination-move");
  // console.log(pagination);
  // width = Number(width);
  // if (height < 2000) {
  if (window.screen.width > 768) {
    pagination.style.left = -width + "px";
  } else if (window.screen.width <= 768) {
    pagination.style.left = "auto";
  }
}

// let anchorWidthBlock = document.querySelector("._position-pag");
let anchorWidthWrap = window
  .getComputedStyle(document.querySelector("._wrapper-width"))
  .width.replace("px", " ");
let roomsImgWidth = window
  .getComputedStyle(document.querySelector("._rooms__img-width"))
  .width.replace("px", " ");
let anchorWidthGap = window
  .getComputedStyle(document.querySelector("._anchor__width-gap"))
  .columnGap.replace("%", " ");
let anchorWidthGapWidth = window
  .getComputedStyle(document.querySelector("._anchor__width-gap"))
  .width.replace("px", " ");
let row1Width = window
  .getComputedStyle(document.querySelector("._row-1-width"))
  .width.replace("px", " ");

row1Width = Number(row1Width);
anchorWidthGap = Number(anchorWidthGap);
anchorWidthGapWidth = Number(anchorWidthGapWidth);
anchorWidthWrap = Number(anchorWidthWrap);
roomsImgWidth = Number(roomsImgWidth);

anchorWidthGap = (anchorWidthGapWidth / 100) * anchorWidthGap;

let finishWidth = anchorWidthWrap - anchorWidthGap - roomsImgWidth - row1Width;

positionPag(finishWidth);
window.addEventListener("resize", function () {
  anchorWidthWrap = window
    .getComputedStyle(document.querySelector("._wrapper-width"))
    .width.replace("px", " ");
  roomsImgWidth = window
    .getComputedStyle(document.querySelector("._rooms__img-width"))
    .width.replace("px", " ");
  anchorWidthGap = window
    .getComputedStyle(document.querySelector("._anchor__width-gap"))
    .columnGap.replace("%", " ");
  anchorWidthGapWidth = window
    .getComputedStyle(document.querySelector("._anchor__width-gap"))
    .width.replace("px", " ");
  row1Width = window
    .getComputedStyle(document.querySelector("._row-1-width"))
